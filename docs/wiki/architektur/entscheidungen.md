...
- **Globaler NavHost**: Core Tabs (Feed, Check-in, Live-Karte, Meldungen, Profil) sind innerhalb eines `HorizontalPager` verpackt für seitliches Swipen, während tiefere Navigation als unabhängige Routen konzipiert ist.
- **Native Karte und getrennter Datenzugriff**: MapLibre rendert OpenFreeMap-Kacheln; Fahrtabschnitte kommen direkt von Transitous. Diese Abfragen verwenden keinen Träwelling-Token. Marker zeigen aus Zeiten und Geometrie geschätzte Positionen an, keine GPS-Messwerte. Siehe [Transitous Live-Karte](../features/transitous-live-karte.md).
- **Projektlizenz**: Der Routely-eigene Quellcode, die Dokumentation und die selbst erstellten App-Grafiken einschließlich Icons stehen unter Apache-2.0 (Datei `LICENSE`). Externe Bibliotheken, Daten und fremde Marken behalten ihre eigenen Rechte; insbesondere gelten die nichtkommerziellen Transitous-API-Bedingungen unabhängig von der Projektlizenz.
